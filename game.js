...
function update(dt){
  ...
  for(let i=beams.length-1;i>=0;i--){ const b=beams[i]; b.t+=dt;
    if(b.t>=0 && b.t<0.45 && !b.hit && state==='play' && Math.abs((player.x+player.w/2)-b.x)<b.w/2+player.w/2){ b.hit=true; damagePlayer(b.x); }
  ...
  if(cfg.type==='collect'&&state==='play'){
    ...
    for(let i=fhearts.length-1;i>=0;i--){ const h=fhearts[i]; h.sw+=dt*4; if(h.fleeT>0) h.fleeT-=dt;
      ...
      else if(near){ h.vx += (Math.sign(dx)*3.2*fast - h.vx)*0.2; }                       // foge
...
function showMenu(){ overlay.classList.remove('hidden');
  ...
  const hint = MOBILE
    ? '<p class="hint">Encoste e arraste em qualquer lugar = <b>joystick</b> (move pros lados).<br>2º dedo: ↑ <b>pulo</b> · ↓ <b>barreira</b> 🧱 · segure ❤ = <b>explosão</b> 💥<br>★ usa o <b>poder</b> · ⇄ troca · ✨ <b>Amor Divino</b> quando a barra enche</p>'
    : '<div class="keys"><b>← →</b> andar · <b>↑</b> pular · <b>ESPAÇO</b> explosão 💥 · <b>↓</b> parede 🧱 · <b>E</b> poder · <b>Q</b> trocar · <b>F</b> Amor Divino ✨</div>';
