...
function saveProgress(){ try{ if(typeof localStorage==='undefined')return;
  localStorage.setItem(SAVE_KEY, JSON.stringify({mode,level,lives,deepest,deaths,ts:Date.now()})); }catch(e){} }
...
function genLevel(n){
  if(n<=5){ const b=HAND[n-1];
    return Object.assign({}, b, {
      ...
      chaserCount:0, shooterCount:0, eaterCount:0, monsterHp:1, monsterScale:1, monsterSpeed:0, shooterRate:2, archCount:0, hue:null, vignette:0,
...
let player, npcs, monsters, shots, platforms, blasts, particles, ambient, stars, walls, fhearts, pHearts, blocks, rock, carrying;
...
function blastMaxR(ch){ return 120 + ch*260 + Math.min((level-1)*18, 520); }
...
function mkSad(x,y){ return { x,y, vx:0, vy:0, w:30, h:44, onGround:false, platRef:null, state:'sad',
  dir:Math.random()<.5?-1:1, wander:rand(40,160), grace:0, freezeT:0, bob:rand(0,6.28),
  phone:Math.random()<0.55, stuck:false, shake:0, heartTrail:0, holding:false }; }
...
function mkMonster(x,y,kind){ const sc=cfg.monsterScale*(kind==='eater'?1.2:(kind==='shooter'?1.05:1));
  return { x,y, w:30*sc, h:42*sc, vx:0, vy:0, onGround:false, platRef:null, kind:kind||'chaser',
  hp:cfg.monsterHp, maxHp:cfg.monsterHp, sp:cfg.monsterSpeed*(kind==='shooter'?0.45:1), hitT:0,
  fireT:rand(0.4,cfg.shooterRate||2), bob:rand(0,6.28), eye:rand(0,6.28), grow:0 }; }
function spawnHeart(x,y,falling){ const big=(cfg.type==='collect'); const s=big?rand(48,66):rand(16,24);
  const h={ x,y, w:s*0.95,h:s*0.95, vx:0, vy: falling?rand(1.2,2.4):0, s,
  col:HEART_COLORS[Math.floor(rand(0,HEART_COLORS.length))], sw:rand(0,6.28), rest:!falling, big }; fhearts.push(h); return h; }
...
function buildLevel(n){
  ...
  npcs=[];monsters=[];shots=[];blasts=[];particles=[];walls=[];ambient=[];stars=[];fhearts=[];pHearts=[];blocks=[];rock=null;carrying=0;rainTimer=0;
  ...
  // plataformas: tijolo denso (tela cheia, vários caminhos) — caminho-base sempre garantido
  platforms=[];
  const nBands=Math.max(4, Math.floor((groundY-110)/phBandH));
  const count=Math.max(3, Math.round(WORLD_W/phSlot));
  const cellW=WORLD_W/count;
  for(let b=0;b<nBands;b++){
    const y=groundY-phBandH*(b+1);
    const off=(b%2)?cellW*0.5:0;
    for(let s=0;s<count;s++){
      const w=rand(88,132), hw=w/2;
      let cx=off+(s+0.5)*cellW+rand(-cellW*0.16,cellW*0.16);
  ...
  for(let i=0;i<Math.min(cfg.moving,movPool.length);i++){ const p=movPool[i];
    ...
    p.mv={x0:p.x-half,x1:p.x+half,sp:cfg.mvSpeed*rand(.7,1.2),dir:Math.random()<.5?-1:1}; }
  ...
  for(let i=0;i<cfg.blocks;i++){ const p=statics[Math.floor(rand(0,statics.length))]||platforms[0];
    const bw=clamp(p.w*rand(.8,1.05),80,132), bh=30, side=Math.random()<.5?-1:1;
  ...
  if(cfg.type==='love'){
    ...
    for(let i=0;i<10;i++) spawnHeart(rand(120,WORLD_W-120), rand(120,groundY-200), true);
    state='play';
  }
  setObjective(); updateHUD();
}

// estruturas surreais; só ADICIONAM superfícies (o caminho-base de tijolo continua alcançável)
function addStructures(count, nBands){
  const yTop=groundY-phBandH*(nBands-1), yBot=groundY-phBandH*1.5;
  const mk=(x,y,w)=>{ const ww=clamp(w,46,150), hw=ww/2, cx=clamp(x,hw+8,WORLD_W-hw-8), yy=clamp(y,yTop,yBot);
    platforms.push({x:cx-hw,y:yy,w:ww,h:14,band:clamp(Math.round((groundY-yy)/phBandH)-1,1,99),cx,hw,mv:null,arch:true}); };
  const kinds=['stair','vee','square','diag','cluster'];
  for(let i=0;i<count;i++){
    const k=kinds[Math.floor(rand(0,kinds.length))];
    const ax=rand(120,WORLD_W-120), ay=rand(yTop+phBandH,yBot), step=phBandH*rand(0.7,0.95), dirx=Math.random()<.5?-1:1;
    if(k==='stair'||k==='diag'){ const kk=Math.floor(rand(3,6)), dx=(k==='diag'?58:70);
      for(let s=0;s<kk;s++) mk(ax+dirx*s*dx, ay-s*step, k==='diag'?60:74); }
    else if(k==='vee'){ const kk=Math.floor(rand(3,5));
      for(let s=0;s<kk;s++){ mk(ax-s*60, ay-s*step, 58); mk(ax+s*60, ay-s*step, 58); } }
    else if(k==='square'){ const sz=rand(120,200), top=ay-sz;
      mk(ax,ay,sz); mk(ax,top,sz); mk(ax-sz/2+10,ay-sz/2,52); mk(ax+sz/2-10,ay-sz/2,52); }
    else { const kk=Math.floor(rand(2,4)); for(let s=0;s<kk;s++) mk(ax+s*92, ay, 96); mk(ax+92, ay-step, 70); }
  }
}
...
function applyVertical(e){
  ...
  for(const p of platforms){ if(e.vy>=0){ const feet=e.y+e.h, prev=feet-e.vy;
...
function defeatMonster(mo){ const cx=mo.x+mo.w/2, cy=mo.y+mo.h/2; shake=Math.min(24,shake+14);
  ...
  const i=monsters.indexOf(mo); if(i>=0) monsters.splice(i,1); }
...
function update(dt){
  ...
  if(state==='play'){
    const slow=player.charging?0.5:1, mm=effMoveMax(), jv=effJump();
  ...
  for(const n of npcs){
    ...
    if(n.state==='sad'){
      if(n.platRef){ if(n.x<=n.platRef.x) n.dir=1; if(n.x>=n.platRef.x+n.platRef.w-n.w) n.dir=-1; n.vx=n.dir*0.7; }
      else { n.wander-=1; if(n.wander<=0){ n.dir=Math.random()<.5?-1:1; if(Math.random()<.3)n.dir=0; n.wander=rand(50,170); }
             n.vx=n.dir*0.9; if(n.x<=0)n.dir=1; if(n.x>=WORLD_W-n.w)n.dir=-1; }
  ...
  // MONSTROS (perseguidor / atirador de corações negros / comedor)
  for(const mo of monsters){ if(mo.hitT>0) mo.hitT-=dt; mo.bob+=dt*5;
    const px=player.x+player.w/2, nx=mo.x+mo.w/2;
    mo.vx += (Math.sign(px-nx)*mo.sp - mo.vx)*0.12;
    if(mo.kind!=='shooter' && mo.onGround && player.y<mo.y-30 && Math.abs(px-nx)<260 && Math.random()<0.04){ mo.vy=phBaseJump*0.8; mo.onGround=false; }
  ...
  for(let i=shots.length-1;i>=0;i--){ const s=shots[i]; s.life+=dt; s.sw+=dt*6; s.x+=s.vx; s.y+=s.vy;
    ...
    if(state==='play' && player.invuln<=0 && overlap(player,{x:s.x-9,y:s.y-9,w:18,h:18})){ shots.splice(i,1); damagePlayer(s.x); } }
  ...
  if(state==='play'){
    ...
    for(const mo of monsters){ if(overlap(player,mo)){ damagePlayer(mo.x+mo.w/2); break; } }
  }

  if(cfg.type==='collect'&&state==='play'){
    rainTimer-=dt; if(rainTimer<=0){ rainTimer=rand(0.25,0.55); if(fhearts.length<40) spawnHeart(rand(60,WORLD_W-60),-30,true); }
    const pcx=player.x+player.w/2;
    for(let i=fhearts.length-1;i>=0;i--){ const h=fhearts[i]; h.sw+=dt*4;
      // FOGE do jogador (mexe em X primeiro)
      const hcx=h.x+h.w/2, dx=hcx-pcx;
      if(Math.abs(dx)<300 && Math.abs((h.y+h.h)-(player.y+player.h))<260){ h.vx += (Math.sign(dx)*3.2 - h.vx)*0.2; }
      else h.vx*=0.85;
      h.x=clamp(h.x+h.vx,2,WORLD_W-h.w-2);
      // se saiu de cima de uma plataforma/chão → CAI (corrige corações flutuando no ar)
      if(h.rest){ const cx=h.x+h.w/2, by=h.y+h.h; let sup=by>=groundY-2;
      ...
      if(overlap(player,hb)){ carrying++; fhearts.splice(i,1); beep(700+carrying*4,0.07,'sine',0.05); updateHUD(); } }
...
function draw(){
  ...
  for(const p of platforms){ if(p.y<camY-30||p.y>camY+H+30) continue;
    ...
    if(p.mv){ ctx.fillStyle='rgba(255,220,150,.7)'; ctx.font='10px sans-serif'; ctx.textAlign='center'; ctx.fillText('↔',p.x+p.w/2,p.y+12); } }
  ...
  if(cfg.type==='collect'){ if(rock) drawRock(rock);
    for(const h of fhearts){ const fl=Math.sin(h.sw)*2; ctx.save(); ctx.shadowColor=h.col; ctx.shadowBlur=10;
      ctx.fillStyle=h.col; heartPath(ctx,h.x+h.w/2,h.y+fl,h.s); ctx.fill();
      ctx.strokeStyle='rgba(255,255,255,.5)';ctx.lineWidth=2;heartPath(ctx,h.x+h.w/2,h.y+fl,h.s);ctx.stroke(); ctx.restore(); } }
...
function drawMonster(mo){ if(mo.y<camY-80||mo.y>camY+H+80||mo.x<camX-80||mo.x>camX+W+80) return;
  ...
  ctx.save(); ctx.shadowColor=PAL?PAL.amb:'#9b30ff'; ctx.shadowBlur=14;
  ctx.fillStyle=mo.hitT>0?'#fff':'#241430'; roundRect(ctx,x,y+bob,w,h,10); ctx.fill();
  ctx.strokeStyle=PAL?PAL.platTop:'#b5179e'; ctx.lineWidth=3; roundRect(ctx,x,y+bob,w,h,10); ctx.stroke(); ctx.restore();
  // olhos
  ctx.fillStyle=mo.kind==='shooter'?'#b14cff':(mo.kind==='eater'?'#ffcc00':'#ff3b6b');
  ctx.beginPath(); ctx.arc(cx-w*0.18,y+bob+h*0.34,w*0.10,0,6.28); ctx.arc(cx+w*0.18,y+bob+h*0.34,w*0.10,0,6.28); ctx.fill();
  if(mo.kind==='eater'){ // boca com dentes
    ctx.fillStyle='#120010'; roundRect(ctx,cx-w*0.3,y+bob+h*0.55,w*0.6,h*0.22,4); ctx.fill();
    ctx.fillStyle='#fff'; for(let i=0;i<4;i++){ const tx=cx-w*0.26+i*w*0.17; ctx.beginPath(); ctx.moveTo(tx,y+bob+h*0.55); ctx.lineTo(tx+w*0.05,y+bob+h*0.66); ctx.lineTo(tx+w*0.10,y+bob+h*0.55); ctx.fill(); }
  } else if(mo.kind==='shooter'){ // canhão apontando pro jogador
    const ang=Math.atan2((player.y+player.h/2)-(y+bob+h*0.4),(player.x+player.w/2)-cx);
    ctx.save(); ctx.translate(cx,y+bob+h*0.4); ctx.rotate(ang); ctx.fillStyle='#3a1c4a'; roundRect(ctx,0,-5,w*0.5,10,3); ctx.fill();
    ctx.fillStyle='#b14cff'; heartPath(ctx,w*0.5,-7,12); ctx.fill(); ctx.restore();
  }
  // espinhos topo
  ctx.fillStyle='#3a1c4a'; for(let i=0;i<4;i++){ const sxp=x+w*(0.18+i*0.22); ctx.beginPath(); ctx.moveTo(sxp,y+bob); ctx.lineTo(sxp+w*0.08,y+bob-h*0.16); ctx.lineTo(sxp+w*0.16,y+bob); ctx.fill(); }
  // barra de vida (resistência à explosão)
  if(mo.maxHp>1){ const bw=w, bx=x, by=y+bob-9; ctx.fillStyle='rgba(0,0,0,.5)'; ctx.fillRect(bx,by,bw,4);
    ctx.fillStyle='#ff2e63'; ctx.fillRect(bx,by,bw*clamp(mo.hp/mo.maxHp,0,1),4); } }
...
function drawNPC(n){ if(n.y<camY-60||n.y>camY+H+60||n.x<camX-60||n.x>camX+W+60) return;
  ...
  if(n.state==='sad'){
    ctx.fillStyle='#3d7bff'; roundRect(ctx,xx,yy,w,h,10); ctx.fill();
    ctx.strokeStyle='#13316e'; ctx.lineWidth=2.5; roundRect(ctx,xx,yy,w,h,10); ctx.stroke();
    ctx.fillStyle='#dfeaff'; ctx.fillRect(cx-7,yy+16,4,2.5); ctx.fillRect(cx+3,yy+16,4,2.5);
    ctx.strokeStyle='#dfeaff'; ctx.lineWidth=2; ctx.beginPath(); ctx.arc(cx,yy+30,4,1.15*Math.PI,1.85*Math.PI); ctx.stroke();
...
function updateHUD(){ const life='HP '+hpStr();
  if(cfg.type==='love'){ const sad=npcs.filter(n=>n.state==='sad').length;
    ...
    countEl.innerHTML='💕 '+(totalSad0-sad)+' / '+totalSad0+'<small>'+life+' · '+sad+' tristes'+(monsters.length?' · 👹'+monsters.length:'')+'<br>⚡ '+jumpLabel()+' · vel +'+vel+'%</small>'; }
  else countEl.innerHTML='🪨 '+(rock?rock.fill:0)+' / '+(rock?rock.target:0)+'<small>'+life+' · 💖 '+carrying+' carregando</small>'; }
function checkWinLove(){ if(cfg.type==='love'&&state==='play'&&npcs.every(n=>n.state!=='sad')) levelClear(); }
...
function levelClear(){ state='end'; overlay.classList.remove('hidden');
  ...
  try{ if(typeof localStorage!=='undefined') localStorage.setItem(SAVE_KEY, JSON.stringify({mode,level:Math.min(level+1,LEVEL_CAP),lives,deepest,deaths,ts:Date.now()})); }catch(e){}
...
function showMenu(){ overlay.classList.remove('hidden');
  ...
  if(cont){ const c=document.getElementById('cont'); c.onclick=()=>{ mode=sv.mode||'adventure'; lives=sv.lives||3; deepest=sv.deepest||sv.level; deaths=sv.deaths||0; startLevel(sv.level); };
    const cl=document.getElementById('clr'); if(cl) cl.onclick=()=>{ clearProgress(); showMenu(); }; }
  document.getElementById('adv').onclick=()=>{ mode='adventure'; lives=3; deaths=0; deepest=1; startLevel(1); };
  document.getElementById('inf').onclick=()=>{ mode='infinite'; deaths=0; deepest=1; startLevel(1); }; }
...
function loop(now){ let dt=(now-last)/1000; last=now; dt=Math.min(dt,0.05);
  if(state==='play') update(dt); if(player) draw(); requestAnimationFrame(loop); }
